...
def draw_static_scene(display, temp_c=None):
    display.fill(0)
    ...
    if temp_c is not None:
        draw_tiny_text(display, "Temp", 2, 60)
        draw_tiny_text(display, "{:.1f}C".format(temp_c), 2, 66)
    else:
        draw_tiny_text(display, "Sensor Error", 2, 60)
    draw_tiny_text(display, "ID:" + STUDENT_ID, 2, 118)
...
while True:
    now = time.time()
    if now - last_temp_time >= 2:
        t, h = read_sensor_safe()
        if t is not None:
            current_temp = t
        last_temp_time = now

    draw_static_scene(oled, current_temp)
