...
try:
    import dht  # type: ignore[import-not-found]
except ImportError:
    dht = None
...
dht_sensor = dht.DHT22(Pin(DHT_PIN)) if dht is not None else None
...
def draw_fireworks(display, frame):
    # Keep fireworks near the dragon-ball side and away from the Chinese text block.
    bursts = (
        (center_x + 20, center_y - 34, 2),
        (center_x + 28, center_y - 14, 1),
        (center_x + 24, center_y + 8, 2),
        (center_x + 30, center_y + 24, 1),
    )
    flicker = (frame % 4) + 1

    for index, (cx, cy, base) in enumerate(bursts):
        size = base + ((frame + index) % 2)
        draw_star(display, cx, cy, size, 1)
        if (frame + index) % 3 == 0:
            display.line(cx, cy - size - 1, cx, cy - size - 3, 1)
            display.line(cx, cy + size + 1, cx, cy + size + 3, 1)
            display.line(cx - size - 1, cy, cx - size - 3, cy, 1)
            display.line(cx + size + 1, cy, cx + size + 3, cy, 1)

        if (frame + index) % 4 == 0:
            # Tiny trailing sparks under some bursts.
            put_pixel(display, cx, cy + size + 4, 1)
            put_pixel(display, cx + 1, cy + size + 5, 1)

    # Sparkle cluster near the top-right gives the area more texture.
    sparkle_x = center_x + 32
    sparkle_y = center_y - 8 + (frame % 4)
    if flicker % 2 == 0:
        put_pixel(display, sparkle_x, sparkle_y, 1)
        put_pixel(display, sparkle_x - 1, sparkle_y, 1)
        put_pixel(display, sparkle_x, sparkle_y + 1, 1)

    if (frame % 3) == 0:
        put_pixel(display, center_x + 34, center_y + 2, 1)
        put_pixel(display, center_x + 33, center_y + 3, 1)

    if (frame % 5) == 0:
        put_pixel(display, center_x + 31, center_y + 20, 1)
        put_pixel(display, center_x + 30, center_y + 21, 1)


def draw_static_scene(display, frame):
    display.fill(0)
    display.hline(0, 0, OLED_WIDTH, 1)
    display.hline(0, OLED_HEIGHT - 1, OLED_WIDTH, 1)
    display.vline(0, 0, OLED_HEIGHT, 1)
    display.vline(OLED_WIDTH - 1, 0, OLED_HEIGHT, 1)
    ...
    draw_circle(display, center_x, center_y, 24, 1)
    draw_circle(display, center_x, center_y, 21, 1)

    pulse = (frame % 4) - 1
    draw_star(display, center_x, center_y, 9 + pulse, 1)
    draw_star(display, 92, 58, 2 + (frame % 2), 1)
...
def draw_chinese_animation(display, frame):
    ...
    for i, char in enumerate(chars):
        ...
        block_size = 32 if enlarged else 16
        x = 96 if enlarged else chinese_base_x
        y = y_cursor + wave[(frame + i) % len(wave)]
        if enlarged:
            y -= 4

        draw_char(display, char, x, y, shrink=shrink)
        y_cursor += block_size + spacing


def draw_temperature(display, temperature, humidity, error, frame):
    ...
    draw_tiny_text(display, "ID", status_x, status_y)
    draw_tiny_text(display, student_id, status_x, status_y + 8)
    draw_tiny_text(display, "TEMP", status_x, status_y + 20)
    draw_tiny_text(display, temp_text, status_x, status_y + 28)
    draw_tiny_text(display, "HUMI", status_x, status_y + 38)
    draw_tiny_text(display, hum_text, status_x, status_y + 46)

    if error:
        if (frame % 4) < 2:
            draw_tiny_text(display, "ERR", status_x, status_y + 58)
    else:
        draw_tiny_text(display, "OK", status_x, status_y + 58)


def read_sensor_safe(sensor):
    if sensor is None:
        print("[ERROR] DHT22 module not available")
        return None, None, "no dht module"
...
def main():
    ...
    while True:
        ...
        if time.ticks_diff(now, last_sensor_ms) >= SENSOR_INTERVAL_MS:
            last_sensor_ms = now
            temp, hum, err = read_sensor_safe(dht_sensor)
            if err is None:
                last_temp = temp
                last_hum = hum
                sensor_error = None
        ...
        draw_temperature(oled, last_temp, last_hum, sensor_error, frame)
