...
def draw_rotating_star(display, frame):
	angle = frame * 0.22
	breath = math.sin(frame * 0.18) * 2
	outer = 13 + int(round(breath))
	inner = max(5, int(round(outer * 0.45)))
	...
	for index in range(10):
		...
		points.append((
			int(round(center_x + math.cos(theta) * radius)),
			int(round(center_y + math.sin(theta) * radius)),
		))
	...
	points = []
	for index in range(10):
		radius = (outer - 2) if index % 2 == 0 else max(4, inner - 1)
		theta = -angle * 1.35 - math.pi / 2 + index * math.pi / 5
		points.append((
			int(round(center_x + math.cos(theta) * radius)),
			int(round(center_y + math.sin(theta) * radius)),
		))
...
def draw_orbit_sparks(display, frame):
	radius = 18
	for index in range(10):
		phase = frame * 0.14 + index * (math.pi / 5)
		x = int(round(center_x + math.cos(phase) * radius))
		y = int(round(center_y + math.sin(phase) * radius))
		display.pixel(x, y, 1)
...
def render_frame(frame_idx):
	...
	draw_orbit_sparks(oled, frame_idx)
	draw_rotating_star(oled, frame_idx)
	draw_dancing_chinese(oled, frame_idx)
	oled.show()
